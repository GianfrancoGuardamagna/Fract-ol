...
void    fractal_init(t_fractal *fractal)
{
	...
	fractal->mlx_window = mlx_new_window(fractal->mlx_connection, SIDE_LEN, SIDE_LEN, "Fract'ol");
	if(!fractal->mlx_window)
		fractal_cleanup(fractal);
	fractal->img.img_ptr = mlx_new_image(fractal->mlx_connection, SIDE_LEN, SIDE_LEN);
	if(!fractal->img.img_ptr)
		fractal_cleanup(fractal);
