...
static int	mandelbrot_iterator(int x, int y, t_fractal *fractal)
{
	...
	z.real = 0;
	z.img = 0;
	c.real = map_pixels(x, -2, 2, 0, SIDE_LEN) * fractal->zoom + fractal->shift_x;
	c.img = map_pixels(y, -2, 2, 0, SIDE_LEN) * fractal->zoom + fractal->shift_y;
    i = 0;
...
static int julia_iterator(int x, int y, t_fractal *fractal)
{
	...
	z.real = map_pixels(x, -2, 2, 0, SIDE_LEN) * fractal->zoom + fractal->shift_x;
	z.img = map_pixels(y, -2, 2, 0, SIDE_LEN) * fractal->zoom + fractal->shift_y;
	...
	while(y < SIDE_LEN)
	{
		...
		while(x < SIDE_LEN)
		{
			...
			if (iter == fractal->max_iterations)
				...
				color = map_pixels(iter, BLACK, BLUE, 0, fractal->max_iterations);
